...
class _FPGARegisterFeatureOOI(object):
    class Codes(object):
        ...
        V1_CONTINUOUS_STROBE_TIMER_INTERVAL_DIVISOR = V3_CONTINUOUS_STROBE_TIMER_MSB = 0x08
...
class SeaBreezeContinuousStrobeFeatureOOI(SeaBreezeContinuousStrobeFeature):
    ...
    def __init__(self, protocol, feature_id, **kwargs):
        super(SeaBreezeContinuousStrobeFeatureOOI, self).__init__(protocol, feature_id, **kwargs)
    ...
    def set_period_micros(self, period_micros):
        ...
        if fpga_major_version == 1:
            ...
            self._fpga.write_register(self._fpga.Codes.V1_CONTINUOUS_STROBE_TIMER_INTERVAL_DIVISOR, timer_interval)
            self._fpga.write_register(self._fpga.Codes.V1_CONTINUOUS_STROBE_BASE_CLOCK_DIVISOR, base_clock)

        elif fpga_major_version == 3:
            counts = period_micros * 48
            if not 0 < counts < 2**32:
                raise ValueError("period_micros is too large")
            self._fpga.write_register(self._fpga.Codes.V3_CONTINUOUS_STROBE_TIMER_MSB, (counts >> 16) & 0xffff)
            self._fpga.write_register(self._fpga.Codes.V3_CONTINUOUS_STROBE_TIMER_LSB, counts & 0xffff)
